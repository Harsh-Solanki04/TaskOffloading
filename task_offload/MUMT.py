...
class MUMT(object):
    def __init__(self,N,M,rand_seed=1):
        ...
        self.Data = pd.concat([self.Datain, self.Dataout.reindex(columns=self.Datain.index)], axis=1)
        #fixed parameters
       ...
       # self.XC = pd.concat([self.X, self.C], axis=1,join_axes=[self.Datain.index])
        self.XC = pd.concat([self.X, self.C], axis=1).reindex(index=self.Datain.index)
